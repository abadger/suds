...
class TestSudokuBoardInit:
    ...
    def test_create_from_old_board(self):
        initial_board = sb.SudokuBoard()
        new_board = sb.SudokuBoard(initial_board)
...
class TestSudokuBoardUpdate:
    ...
    def test_update_failure(self, board):
        with pytest.raises(sb.InvalidBoardPosition):
            board.update({(0, 1): 1})
